...
breakdowns_summary.each do |key, value|
	amount_of_towns = valuep[:return].length
...
towns.each do |value|
	...
	if total_breakdowns[:get_breakdowns_by_town_or_city_response][:return].kind_of?(Array)
		for breakdowns in 0...total_breakdowns[:get_breakdowns_by_town_or_city_response][:return].length
			puts "***************************************"
			puts "Pueblo: " + total_breakdowns[:get_breakdowns_by_town_or_city_response][:return][breakdowns][:r1_town_or_city]
		...
		puts "***************************************"
		puts "Pueblo: " + total_breakdowns[:get_breakdowns_by_town_or_city_response][:return][:r1_town_or_city]
